...
print('Missing Values: ' +
      str(dataset.shape[0] - dataset.isnull().shape[0]) + '\n')
...
def displots_custom(list, suptitle):
    ...
    for i, data in enumerate(list):
        sns.kdeplot(data, color='m',
                    fill=True, ax=axes[i]).set_title(suptitle[i])
...
displots_custom([amount_value, time_value],
                ['Distribution of Amount', 'Distribution of Time'])

# checks to see any notable differences in valid and fraudulent
print('Average Amount in a Fraudulent Transaction: ' +
      str(dataset[dataset['Class'] == 1]['Amount'].mean()))
print('Average Amount in a Valid Transaction: ' +
      str(dataset[dataset['Class'] == 0]['Amount'].mean()))
